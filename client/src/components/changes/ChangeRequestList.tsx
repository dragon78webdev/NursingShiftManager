...
const ChangeRequestList = ({ statusFilter }: ChangeRequestListProps) => {
  ...
  const filteredRequests = statusFilter 
    ? changeRequests.filter(request => request.status === statusFilter)
    : changeRequests;
      ...
      <div className="overflow-x-auto">
        {filteredRequests.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            ...
            {statusFilter === 'approved' && " approvate"}
            {statusFilter === 'rejected' && " rifiutate"}
          </div>
        ) : (
          <Table>
            <TableHeader>
